...
<div class="content">
    ...
    <div class="dashboard-content">
        ...
        $products = [
            ...
            ["Skirt", ["S", "M", "L"], ["Red", "Blue"], "Zara", 0, 15], // Out of Stock example
            ["Crop Top", ["XS", "S", "M"], ["White", "Pink"], "H&M", 8, 12], // Low Stock
       ...
       <!-- Total Products Card with Click-to-Show -->
<div class="card" id="totalProductsCard" style="background: #4CAF50; color: white;">
    ✅ Total Products<br>
    <?php echo $totalProducts; ?>
</div>

<div id="totalProductsTable" style="display: none; grid-column: span 3; background: white; padding: 20px; border-radius: 10px;">
    <h3>📃 All Products</h3>
    <table>
        <thead>
        <tr>
            ...
            <div id="productTable" style="display: none; margin-top: 15px;">
                <table>
                    ...
                    foreach ($products as $index => $item) {
                        ...
                        $total = $item[4] * $item[5];
                        $status = $item[4] == 0 ? "Out of Stock" : ($item[4] <= 10 ? "Low Stock" : "In Stock");

                        echo "<tr id='$productId'>";
                        echo "<td>{$item[0]}</td>";

                        echo "<td><select onchange=\"updateProductDetails('$productId')\" class='size-dropdown'>
                        ...
                        echo "<td><select class='color-dropdown'><option value=''>Select size first</option></select></td>";
                        echo "<td>{$item[3]}</td>";
                        echo "<td class='qty'>{$item[4]}</td>";
                        echo "<td class='price'>₱" . number_format($item[5], 2) . "</td>";
...
<script>
    const productData = {
        product_0: { sizes: ["S", "M", "L"], colors: ["Black", "White"], qty: 50, price: 10 },
        product_1: { sizes: ["28", "30", "32"], colors: ["Blue", "Black"], qty: 30, price: 25 },
        product_2: { sizes: ["S", "M", "L"], colors: ["Red", "Blue"], qty: 0, price: 15 },
        product_3: { sizes: ["XS", "S", "M"], colors: ["White", "Pink"], qty: 8, price: 12 },
    ...
    function updateProductDetails(productId) {
        ...
        if (!selectedSize) {
            colorDropdown.innerHTML = "<option>Select size first</option>";
            return;
        }

        const colors = productData[productId].colors;
        ...
        status.textContent = quantity == 0 ? "Out of Stock" : (quantity <= 10 ? "Low Stock" : "In Stock");
    }

document.getElementById("totalProductsCard").addEventListener("click", function () {
    const table = document.getElementById("totalProductsTable");
